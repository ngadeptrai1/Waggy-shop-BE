...
public class OrderServiceImpl implements OrderService {
      ...
      order =   orderRepository.save(order) ;
        ...
        for (OrderDetailDTO detail:
            ...
            Product pr = productRepo.findById(detail.getProductId()).orElseThrow(
                    ()->{ return new DataNotFoundException( "Khong tim thay san pham id = " + detail.getProductId());});
    ...
    public OrderResponse findById(Long id) {
        Order order =  orderRepository.findById(id).orElseThrow
                (()->{ return new DataNotFoundException("Khong tim thay order cos id = "+id);});
        modelMapper.typeMap(Order.class,OrderResponse.class);
        return modelMapper.map(order,OrderResponse.class);
    }
    ...
    public OrderResponse update(OrderDTO order,long id) {
        Order existingOrder = orderRepository.findById(id).orElseThrow(()->{
            return new DataNotFoundException("khon gtim thay order id = "+id);
        });
        User u = userRepo.findById(order.getUserId()).orElseThrow(()->{
            return  new DataNotFoundException("khong tim thay user id  = "+order.getUserId());
        });
        modelMapper.typeMap(OrderDTO.class,Order.class)
                .addMappings(mapper ->mapper.skip(Order::setId));
    ...
    public OrderResponse delete(Long id) {
        Order existingOrder = orderRepository.findById(id).orElseThrow(()->{
            return new DataNotFoundException("khong tim thay order id = "+id);
        });
